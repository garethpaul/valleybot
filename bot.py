...
def find_pronoun(sent):
    ...
    for word, part_of_speech in sent.pos_tags:
        ...
        if part_of_speech == 'PRP' and word.lower() == 'you':
            ...
            # If the user mentioned themselves, then they will definitely be
            # the pronoun
            pronoun = 'You'
    return pronoun
...
def construct_response(pronoun, noun, verb):
    ...
    if verb:
        ...
        if verb_word in (
            ...
            'am',
            'is',
                "'m"):  # This would be an excellent place to use lemmas!
            if pronoun.lower() == 'you':
                # The bot will always tell the person they aren't whatever they
                # said they were
    ...
    if pronoun == 'I' and (noun or adjective):
        if noun:
            if random.choice((True, False)):
                resp = random.choice(config.SELF_VERBS_WITH_NOUN_CAPS_PLURAL).format(
                    **{'noun': noun.pluralize().capitalize()})
            else:
                resp = random.choice(
                    config.SELF_VERBS_WITH_NOUN_LOWER).format(**{'noun': noun})
        else:
            resp = random.choice(config.SELF_VERBS_WITH_ADJECTIVE).format(
                **{'adjective': adjective})
    return resp
...
def respond(sentence):
    ...
    if not resp:
        ...
        if not pronoun:
            ...
            resp = random.choice(config.COMMENTS_ABOUT_SELF)
        else:
